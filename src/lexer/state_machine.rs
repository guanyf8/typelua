...
impl StateMachine {
    ...
    pub fn next_state(&mut self, c: char) -> (Step, u32) {
        let (step, hint): (Step, u32) = match self.current {
            State::S0 => match c {
                ...
                op @ ('+' | '*' | '%' | '^' | '#' | '&' | '|' | '(' | ')' | '{' | '}' | ']'
                | ';' | ',') => (Step::ACCEPT(AcceptType::OPERATOR(OpType::SIMPLE(op))), 0),
            ...
            State::LBKT(n, cmt) => {
                match c {
                    ...
                    _ => {
                        if cmt {
                            (Step::CONTINUE(State::SCMT), 1)
                        } else {
                            (
                                Step::ACCEPT(AcceptType::OPERATOR(OpType::SIMPLE('['))),
                                n + 1,
                            )
                        }
                    }
        ...
        self.current = if let Step::CONTINUE(s) = step {
            s
        } else {
            State::S0
        };
    ...
    pub fn finish(&mut self) -> Finish {
        let result = match self.current {
            ...
            State::LSTR(_, false) | State::LCLOSE(_, _, false) => {
                Finish::Error(LexErr::UnclosedLongStr)
            }
            State::LSTR(_, true) | State::LCLOSE(_, _, true) => {
                Finish::Error(LexErr::UnclosedLongComment)
            }
            State::DQ | State::SQ | State::DQESC | State::SQESC => {
                Finish::Error(LexErr::UnclosedString)
            }
            State::ZERO
            | State::HEXI
            | State::HEXF
            ...
            | State::INT
            | State::FRAC
            | State::EXP => Finish::Token(AcceptType::NUMERAL, 0),
