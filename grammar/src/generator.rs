...
impl Grammar {
    ...
    pub fn build_grammar(input: TokenStream) -> Grammar {
        ...
        let start_alternatives = grammar
            .rules
            .iter()
            .filter(|rule| rule.left == start)
            .count();
        ...
        while changed {
            ...
            for rule in &self.rules {
                ...
                for symbol in &rule.right {
                    let symbol_first: Vec<i32> =
                        self.firsts[*symbol as usize].iter().copied().collect();
                    for first in &symbol_first {
                        if *first != -1 {
                            //todo 因为数据表示选型，非常容易写错，后面可以想想怎么优化
    ...
    pub fn get_seq_first(&self, seq: &[u32], lookahead: u32) -> HashSet<u32> {
        ...
        for symbol in seq {
            ...
            seq_first.extend(
                symbol_first
                    .iter()
                    .filter_map(|&f| if f != -1 { Some(f as u32) } else { None }),
            );
            if !symbol_first.contains(&-1) {
                nullable = false;
                break;
    ...
    fn end_symbol(&self) -> u32 {
        self.get_index(END_SYMBOL)
            .expect("END_SYMBOL 应在 build_grammar 中加入")
    }
    ...
    P: WrapItem<T>,
{
    ...
    while let Some(i) = todo_list.pop() {
        ...
        let Some(&symbol) = rule.right.get(i.position) else {
            continue;
        };
...
fn build_kernels(
    ...
    ItemsCollection<()>,
    HashMap<(u32 /*state */, u32 /* symbol */), u32 /* trans_state */>,
) {
    ...
    while pos < kernel_collection.len() {
        ...
        let concern_symbols: BTreeSet<&u32> = items_closure
            .iter()
            .filter_map(|item| {
                let rule = &grammar.rules[item.rule_index];
...
fn build_lookaheads(
    ...
    for (i, items) in kernels.iter().enumerate() {
        for item in items {
            let lalr_closure = build_closure(
                Item {
            ...
            for closure_item in lalr_closure {
                let Some(&concern_symbol) = grammar.rules[closure_item.rule_index]
                    .right
                    .get(closure_item.position)
                else {
                    continue;
                };
                ...
                if closure_item.lookahead == PROPAGATE_MARK {
                    ...
                    spontaneous[goto_index as usize]
                        .get_mut(&target_item)
                        .unwrap()
    ...
    debug_assert!(
        spontaneous
            .iter()
            .all(|m| m.values().all(|s| !s.contains(&PROPAGATE_MARK))),
        "# 外泄到前看集里了"
    );
...
fn resolve_lookaheads(mut spon: Lookaheads, prop: &Propagates) -> Lookaheads {
    let mut todo_list: Vec<(usize, Item0, u32)> = spon
        .iter()
        .enumerate()
        .flat_map(|(i, from_kernel)| {
            from_kernel
                .iter()
                .flat_map(move |(&from_item, lookaheads)| {
                    lookaheads
                        .iter()
                        .map(move |&lookahead| (i, from_item, lookahead))
                })
        })
        .collect();

    while let Some((kernel_index, from_item, lookahead)) = todo_list.pop() {
        let Some(targets) = prop[kernel_index].get(&from_item) else {
            continue;
        };
...
impl ParseTable {
    pub fn generate_parse_table(grammar: &Grammar) -> ParseTable {
        ...
        let lalr_items = kernels_lalr
            .iter()
            .map(|kernel| build_closure(kernel.clone(), grammar))
        ...
        let mut parse_table = ParseTable {
            ...
            rule_rhs_len: grammar
                .rules
                .iter()
                .map(|rule| rule.right.len() as u32)
                .collect(),
            ...
            for item in items {
                ...
                if item.rule_index == 0 && item.lookahead == grammar.end_symbol() {
                    ...
                    set_action(
                        ...
                        i as u32,
                        item.lookahead,
                        Action::Reduce(item.rule_index as u32),
                    );
                }
            }
        }
